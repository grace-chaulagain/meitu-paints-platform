...
import { DispatcherOwnOrderDetailMobileView } from "../../mobile/DispatcherOwnOrderDetailMobileView.jsx";

// Routed detail page for a dispatcher's own replenishment orders (buying
// stock from the Factory) - mirrors DealerOrderDetailPage.jsx exactly (same
// .dealer-order-detail-* layout/styles, same GhostButton Print/Download
// pair) so a dispatcher's own order history looks and behaves identically
// to a dealer's, just with no dealer to show (DeliveryDetailsCard already
// no-ops when order.dealerSnapshot is empty, which it is for these orders).
export default function DispatcherOwnOrderDetailPage() {
  ...
  function goBack() {
    // Same "restore the list's exact state" pattern as
    // DispatcherOrderDetailPage.jsx's assigned-dealer-orders detail page.
  ...
  if (isMobile) {
    return (
      <DispatcherOwnOrderDetailMobileView
        order={order}
        loading={orderQuery.isLoading}
        loadError={orderQuery.error ? getQueryErrorMessage(orderQuery.error, "Failed to load this order.") : !order && !orderQuery.isLoading ? "This order could not be found." : ""}
        onBack={goBack}
          ...
          <div className="dealer-order-detail-main">
            ...
            {(order.dealerNote || order.internalNote) ? (
              <Surface padding={14} style={{ marginTop: 16, display: "grid", gap: 8 }}>
                {order.dealerNote ? (
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".04em", color: "var(--color-graphite, #707070)" }}>Note</div>
