...
export default function DispatcherOrderCatalogPage() {
  ...
  // DispatcherCatalogMobileView.jsx's drilldownCategory exactly, and the
  // desktop DealerCatalogPage.jsx equivalent) rather than a real route
  // change - the root view underneath never unmounts, so its
  ...
  // fix as DispatcherCatalogMobileView.jsx's applyScrollTop).
  ...
  return (
    <div className="dispatcher-catalog-shell">
      ...
      >
        {drilldownCategory ? (
          <>
            <button type="button" className="dispatcher-catalog-drilldown-back" onClick={closeCategoryDrilldown}>
            ...
            <div>
              <div className="dispatcher-category-heading">
                ...
                <span className="dispatcher-category-count">{drilldownCategory.items.length}</span>
              </div>
              <div className={viewMode === "list" ? "dispatcher-catalog-list" : "dispatcher-catalog-grid"}>
            ...
            {loading ? (
              ...
              <div style={{ display: "grid", gap: 40 }}>
                {familyGroups.map((group) => {
                  ...
                  // once a specific category is already chosen in the
                  // dropdown, familyGroups has exactly one entry matching it,
                  // so there's nothing left to drill into.
                  ...
                  return (
                    <div key={group.key || "uncategorized"}>
                      {isDrillable ? (
                        <button
                          type="button"
                          className="dispatcher-category-heading is-drillable"
                          onClick={() => openCategoryDrilldown(group)}
                        >
                          <span className="dispatcher-category-heading-label">{group.label}</span>
                          <DashboardIcon name="chevron" size={13} strokeWidth={2.4} className="dispatcher-category-heading-chevron" />
                      ...
                      {viewMode === "list" ? (
                        ...
                        <div className="dispatcher-catalog-grid">{cards}</div>
                      )}
                    </div>
                  );
                })}
      ...
      <style>{`
        ...
        .dispatcher-category-heading.is-drillable{
          cursor:pointer;
          border-radius:10px 10px 0 0;
          transition:background-color .14s ease, transform .12s var(--ease-out, ease);
        }
        .dispatcher-category-heading.is-drillable:hover{
          background:rgba(29,29,31,.03);
        }
        .dispatcher-category-heading.is-drillable:active{
          transform:scale(.995);
        }
        .dispatcher-category-heading-chevron{
          flex-shrink:0;
          color:var(--color-azure, #0071e3);
          transition:transform .16s var(--ease-out, ease);
        }
        .dispatcher-category-heading.is-drillable:hover .dispatcher-category-heading-chevron{
        ...
        .dispatcher-category-heading-label{
          position:relative;
          display:inline-block;
        }
        .dispatcher-category-heading-label::after{
          content:"";
          position:absolute;
          left:0;
          right:0;
        ...
        .dispatcher-category-heading.is-drillable:hover .dispatcher-category-heading-label{
          color:var(--color-azure, #0071e3);
        }
        .dispatcher-category-heading.is-drillable:hover .dispatcher-category-heading-label::after{
          transform:scaleX(1);
        }

        .dispatcher-catalog-drilldown-back{
          display:inline-flex;
          align-items:center;
          gap:8px;
          width:fit-content;
        ...
        .dispatcher-catalog-drilldown-back:hover{
          color:var(--color-azure, #0071e3);
        }
        .dispatcher-catalog-drilldown-back:active{
          transform:scale(.97);
        }
