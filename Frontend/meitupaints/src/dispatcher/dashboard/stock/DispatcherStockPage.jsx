...
const HISTORY_TYPE_OPTIONS = [
  ...
  { key: "REPLENISHMENT_IN", label: "Received" },
  { key: "DISPATCH_OUT", label: "Dispatched" },
];
...
function sortItems(items, sort) {
  const sorted = [...items];
  if (sort === "stock-asc") sorted.sort((a, b) => a.currentQuantity - b.currentQuantity);
  else if (sort === "stock-desc") sorted.sort((a, b) => b.currentQuantity - a.currentQuantity);
  else if (sort === "value-desc") sorted.sort((a, b) => (b.inventoryValue || 0) - (a.inventoryValue || 0));
  else if (sort === "recent") sorted.sort((a, b) => new Date(b.lastUpdatedAt || 0) - new Date(a.lastUpdatedAt || 0));
...
function CategorySectionHeading({ label, count }) {
  return (
    <div className="dispatcher-inv-category-heading">
      <span>{label}</span>
      <span className="dispatcher-inv-category-count">{count}</span>
    </div>
  );
}
...
function InventoryStatCard({ icon, label, value, tone = "neutral" }) {
  return (
    <div className="dispatcher-inv-stat">
      <span className={`dispatcher-inv-stat-icon dispatcher-inv-stat-icon--${tone}`}>
        <DashboardIcon name={icon} size={18} strokeWidth={1.8} />
      </span>
      <div>
        <div className="dispatcher-inv-stat-label">{label}</div>
        <div className="dispatcher-inv-stat-value">{value}</div>
      </div>
    </div>
  );
}

function InventoryProductCard({ item }) {
  const image = getPrimaryImage(item.images);
  const status = computeStatus(item);
  return (
    <Surface padding={0} style={{ overflow: "hidden" }} className="dispatcher-inv-card">
      <div className="dispatcher-inv-card-media">
        {image?.url ? (
      ...
      <div style={{ padding: "14px 16px 16px" }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--color-ink, #1d1d1f)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {item.name}
        </div>
        <div style={{ marginTop: 2, fontSize: 12, color: "var(--color-graphite, #707070)" }}>
          {item.pack?.label || categoryLabel(item.category) || "Product"}
        </div>
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, paddingTop: 12, borderTop: "1px solid rgba(0,0,0,.06)" }}>
          ...
          <span
            style={{
              ...
              fontSize: 12,
              fontWeight: 600,
              color: statusTone(status) === "critical" ? "#b42318" : statusTone(status) === "caution" ? "var(--color-caution, #b64400)" : "#15803d",
            }}
          >
...
// Same underline-tab treatment as the Dealer Inventory page's Inventory/
// History switch (.dealer-inv-section-tab), reused here under a
// "dispatcher-" prefix per this codebase's convention.
function InventorySectionTabs({ value, onChange }) {
  return (
    <div className="dispatcher-inv-section-tabs" role="tablist">
      {SECTION_TABS.map((option) => {
        ...
        return (
          <button
            ...
            className={`dispatcher-inv-section-tab ${active ? "is-active" : ""}`}
          >
...
function InventoryHistoryTable({ query, page, totalPages, onPageChange }) {
  ...
  // oldest-first when the Newest First toggle is flipped), so grouping
  // preserves whichever order that is.
  ...
  if (items.length === 0) {
    return <EmptyState icon="history" title="No stock movements found" subtitle="Replenishment orders and dealer dispatches that change your stock will show up here." />;
  }
...
export default function DispatcherStockPage() {
  ...
  const summary = useMemo(() => {
    ...
    for (const item of allItems) {
      ...
      if (item.inventoryValue) totalValue += Number(item.inventoryValue || 0);
      const itemStatus = computeStatus(item);
      if (itemStatus === "LOW_STOCK") lowStockCount += 1;
      if (itemStatus === "OUT_OF_STOCK") outOfStockCount += 1;
    }
    return { totalValue, totalUnits, lowStockCount, outOfStockCount, totalProducts: allItems.length };
  }, [allItems]);
  ...
  const filteredItems = useMemo(() => {
    ...
    if (status !== "ALL") list = list.filter((item) => computeStatus(item) === status);
    if (q) {
  ...
  }, [allItems, activeCategory, status, query, sort]);
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <Surface padding={16} className="dash-fade-up dispatcher-inv-toolbar">
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <SectionHeader
            icon="stock"
            title="My Stock"
            size="large"
            subtitle="On-hand quantities at your regional warehouse."
        ...
        {section === "inventory" && filterOpen ? (
          <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <Dropdown value={status} options={STATUS_OPTIONS} onChange={setStatus} style={{ width: 160 }} />
            <Dropdown value={sort} options={SORT_OPTIONS} onChange={setSort} style={{ width: 180 }} />
          </div>
        ) : null}
          ...
          <>
            <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8 }}>
              ...
              <InventoryStatCard
                icon="stock"
                label="Total Stock"
                value={<>{summary.totalUnits.toLocaleString()} <span style={{ fontSize: 13, fontWeight: 600, color: "var(--color-graphite, #707070)" }}>Units</span></>}
              />
              <InventoryStatCard icon="invoice" label="Estimated Value" value={formatMoney(summary.totalValue)} tone="accent" />
            </div>

            <div className="dispatcher-inv-category-filter" style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
      ...
      {section === "history" ? (
        ...
        <EmptyState
          icon="stock"
          title={allItems.length === 0 ? "No stock on hand yet" : "No products match this search"}
          subtitle={allItems.length === 0 ? "Place an order from My Order to get started." : ""}
        />
      ) : view === "card" ? (
        <>
              ...
              <div key={group.key || "uncategorized"}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
                  {group.items.map((item) => (
                    <InventoryProductCard key={item.productId} item={item} />
                  ))}
                </div>
              </div>
            ))}
          ...
          {visibleCount < filteredItems.length ? (
            <div style={{ display: "flex", justifyContent: "center", marginTop: 20 }}>
              <GhostButton onClick={() => setVisibleCount(filteredItems.length)}>Show more</GhostButton>
            </div>
          ) : null}
        </>
      ) : (
            ...
            <div key={group.key || "uncategorized"}>
              ...
              <Surface padding={0}>
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns: "minmax(0,1fr) 90px 90px 110px 110px",
                    gap: 12,
                    padding: "10px 16px",
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span><Pill tone={statusTone(itemStatus)} size="small">{statusLabel(itemStatus)}</Pill></span>
                      <span style={{ fontSize: 11.5, fontWeight: 500, color: "var(--color-graphite, #707070)" }}>{timeAgo(item.lastUpdatedAt)}</span>
                    </div>
                  );
                })}
              </Surface>
      ...
      <style>{`
        .dispatcher-inv-category-heading{
          display:flex;
          align-items:center;
          gap:8px;
          margin-bottom:14px;
        ...
        .dispatcher-inv-category-filter .apple-dropdown-menu-trigger{
        ...
        .dispatcher-inv-category-filter .apple-dropdown-menu-trigger:hover{
          background:#000;
        }

        .dispatcher-inv-category-filter .apple-dropdown-menu-trigger:focus-visible{
          outline:2px solid rgba(0,113,227,.42);
          outline-offset:3px;
        }

        .dispatcher-inv-toolbar .dash-section-heading{
          gap:8px;
        }

        .dispatcher-inv-toolbar .dash-section-title{
          font-size:24px!important;
          line-height:1.12!important;
        }

        .dispatcher-inv-toolbar .dash-section-subtitle{
        ...
        .dispatcher-inv-stat-icon--accent{
          background:rgba(0,113,227,.1);
          color:var(--color-azure, #0071e3);
        }
        .dispatcher-inv-stat-label{
          font-size:11.5px;
          font-weight:600;
          color:var(--color-graphite, #707070);
        }
        .dispatcher-inv-stat-value{
          margin-top:1px;
          font-size:17px;
          font-weight:800;
          letter-spacing:-.02em;
        ...
        .dispatcher-inv-card:hover{
          transform:translateY(-2px);
          box-shadow:0 12px 28px rgba(15,23,42,.08);
        }
        .dispatcher-inv-card-media{
          position:relative;
          height:150px;
          display:grid;
          place-items:center;
        ...
        .dispatcher-inv-section-tabs{ display:flex; align-items:center; gap:24px; border-bottom:1px solid rgba(29,29,31,.1); overflow-x:auto; scrollbar-width:none; }
        .dispatcher-inv-section-tabs::-webkit-scrollbar{ display:none; }
        .dispatcher-inv-section-tab{ min-width:max-content; display:flex; align-items:center; gap:10px; padding:10px 4px 13px; border:none; background:transparent; cursor:pointer; border-bottom:3px solid transparent; margin-bottom:-1px; white-space:nowrap; }
        .dispatcher-inv-section-tab span{ font-size:13.5px; font-weight:650; color:var(--color-graphite, #707070); }
        .dispatcher-inv-section-tab.is-active{ border-bottom-color:var(--color-azure, #0071e3); }
        .dispatcher-inv-section-tab.is-active span{ color:var(--color-azure, #0071e3); font-weight:700; }
      `}</style>
    </div>
  );
}
