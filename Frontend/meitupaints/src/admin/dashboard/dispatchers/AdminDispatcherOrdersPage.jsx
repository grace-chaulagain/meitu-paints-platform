...
function statusTone(status) {
  ...
  if (s === "VERIFIED" || s === "COMPLETED") return "positive";
  ...
  return (
    ...
    >
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ marginTop: 2, fontSize: 11.5, color: "var(--color-graphite, #707070)" }}>{Array.isArray(order.items) ? `${order.items.length} items` : "—"}</div>
      </div>
...
function OrderDetailModal({ order, dispatcher, onClose }) {
    ...
    >
      <Surface className="dash-modal-surface-in" style={{ width: "min(920px, 100%)", maxHeight: "90vh", overflow: "auto" }} padding={22} onClick={(event) => event.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
          <SectionHeader eyebrow={dispatcher?.name || dispatcher?.companyName || "Dispatcher"} icon="orders" title={order.orderNumber || "Order Detail"} />
        ...
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "minmax(0,1.1fr) minmax(240px,.9fr)", gap: 16 }}>
          ...
          <Surface padding={16} style={{ display: "grid", gap: 12 }}>
            <DetailItem label="Dispatcher" value={dispatcher?.name || dispatcher?.companyName} />
            {order?.dealerNote ? <DetailItem label="Note" value={order.dealerNote} /> : null}
...
export default function AdminDispatcherOrdersPage() {
  const navigate = useNavigate();
  const location = useLocation();

  const dispatcherId = useMemo(() => {
    const match = location.pathname.match(/^\/admin\/dashboard\/dispatchers\/([^/]+)\/orders$/);
    return match?.[1] || "";
  }, [location.pathname]);
  ...
  const ordersQuery = useGetAdminDispatcherOwnOrdersQuery({ dispatcherId, limit: 200 }, { skip: !dispatcherId });

  const dispatcher = dispatcherQuery.data?.item || null;
  const allOrders = useMemo(() => ordersQuery.data?.items || [], [ordersQuery.data]);

  const loading = dispatcherQuery.isLoading && ordersQuery.isLoading && allOrders.length === 0;
  const isRefreshing = Boolean(dispatcher || allOrders.length) && (dispatcherQuery.isFetching || ordersQuery.isFetching);
  const queryError = dispatcherQuery.error?.message || ordersQuery.error?.message || "";
  ...
  const searchedOrders = useMemo(() => {
    ...
    return allOrders.filter((order) =>
      [order.orderNumber, order.dealerNote, order.internalNote].filter(Boolean).some((value) => String(value).toLowerCase().includes(q)),
    );
  }, [allOrders, search]);

  const filteredOrders = useMemo(() => {
    if (statusFilter === "ALL") return searchedOrders;
    if (statusFilter === "ARCHIVED") return searchedOrders.filter((order) => Boolean(order.archivedAt));
    return searchedOrders.filter((order) => String(order.status || "").toUpperCase() === statusFilter);
  ...
  const countsByFilter = useMemo(() => {
    return {
      ...
      ARCHIVED: allOrders.filter((o) => o.archivedAt).length,
    };
  }, [allOrders]);
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Surface padding={18} className="dash-fade-up">
        <SectionHeader
          icon="orders"
          title="Order History"
          subtitle={dispatcher?.name || dispatcher?.companyName || ""}
          action={
        ...
        <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={{ maxWidth: 320, flex: "1 1 240px" }}>
            <SearchField value={search} onChange={setSearch} placeholder="Search order number, notes…" />
          </div>
          <SegmentedControl options={filterOptions} value={statusFilter} onChange={setStatusFilter} />
        </div>

        {queryError ? (
          <div style={{ marginTop: 14, padding: "12px 14px", borderRadius: 12, background: "rgba(180,35,24,.08)", color: "#b42318", fontSize: 13, fontWeight: 600 }}>{queryError}</div>
        ) : null}
      </Surface>
      ...
      {loading ? (
        ...
        <Surface padding={20}>
          <EmptyState
            ...
            subtitle={dispatcher?.name ? `No matching replenishment orders were found for ${dispatcher.name}.` : "No matching replenishment orders were found for this dispatcher."}
          />
      ...
      <OrderDetailModal order={activeOrder} dispatcher={dispatcher} onClose={() => setActiveOrder(null)} />
    </div>
  );
}
