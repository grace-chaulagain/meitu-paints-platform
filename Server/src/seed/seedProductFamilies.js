...
const dataPath = path.join(__dirname, "meitu-productsFamily.json");
...
const buildFamilyDocs = (parsed) => {
  if (!Array.isArray(parsed)) {
    throw new Error("Family seed file must be a JSON array");
  }

  const seenCodes = new Set();

  return parsed.map((family, index) => {
    ...
    if (seenCodes.has(code)) {
      throw new Error(`Duplicate family code found: ${code}`);
    }
    seenCodes.add(code);
...
const seedProductFamilies = async () => {
  try {
    ...
    assertSafeDatabaseWrite({
      mongoUri,
      operation: "full product family seed",
      destructive: true,
    });
    ...
    console.log(`Found ${familyDocs.length} product family documents`);

    await ProductFamily.deleteMany({});
    console.log("Old product families removed");

    await ProductFamily.insertMany(familyDocs);
    console.log("Product families seeded successfully 🎉");

    process.exit(0);
  } catch (error) {
    console.error("Family seeding failed ❌");
